...
def _collect_delta_issues(change_dir):
    ...
    for spec_file in _find_delta_spec_files(specs_dir):
        ...
        entry = _rel_under(spec_file, specs_dir)
...
def new_change(root, name):
    """创建 change 目录（镜像 ``openspec new change`` 的产物）。

    产物：``openspec/changes/<name>/.openspec.yaml``（schema + created=UTC 日期），
    并保证 openspec/specs、openspec/changes/archive 存在；config.yaml 缺失时按
    CLI 同款最小内容 ``schema: spec-driven`` 创建（注意与 ensure_config 的完整
    模板不同——先 ensure_config 后 new_change 则维持完整模板不动）。
    """
    root = os.path.abspath(root)
...
def _count_tasks(change_dir):
    """镜像 getTaskProgressForChange 的有效行为：数 tasks.md 顶层复选框。"""
    tasks_path = os.path.join(change_dir, "tasks.md")
    try:
        content = _read_text(tasks_path)
    except OSError:
...
def instructions(root, artifact, change):
    ...
    if selected is None:
        raise SpecEngineError(
            ...
            % (artifact, schema_name, ", ".join(valid_ids)))
...
def status(root, change):
    """返回四类制品的存在性/就绪态 + 主 specs 域清单 + 任务进度。"""
    root = os.path.abspath(root)
    change_dir = _require_change_dir(root, change)
    schema_name = _resolve_schema_name(root, change_dir, strict=True)
    schema = _load_schema(schema_name)
...
def _find_spec_updates(change_dir, main_specs_dir):
    """镜像 findSpecUpdates：只认一层 specs/<域>/spec.md（嵌套层不合并，与 CLI 一致）。"""
    updates = []
    specs_dir = os.path.join(change_dir, "specs")
    try:
...
def archive(root, change, date=None):
    ...
    for update in _find_spec_updates(change_dir, main_specs):
        source_content = _read_text(update["source"])
