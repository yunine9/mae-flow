...
1. 完整流程与独立模式都直接使用任务卡冻结的首检文件、批次和告警，禁止按语言经验重算或扩大范围。
   文件清单只是 CLI 扫描输入；真正可修范围以任务卡「CodeCheck修复目标（硬边界）」逐条告警为准。
   **同一文件里未列出的存量告警不得顺手修复**。执行 fullcheck → 能识别则读落盘报告拿告警清单；
   格式未知时保留原始结果，不猜数字
2. 逐条修复可自动修复的告警
3. 修复后**按配置的编译方式重新编译验证**：配置是 build-fix → 用 Skill 工具调用插件自带能力；
   ...
   不转后台或轮询。**禁止自行另猜编译命令**。编译从任务卡的“修复后编译执行目录”运行，
   多模块分别验证。目录未确定时如实 FAIL，
   禁止退回项目根做全仓编译。CodeCheck `fullcheck` 本身仍按上文要求固定从项目根运行。
4. **只重跑一轮 fullcheck 复验**。分批时逐批保留机器可得的计数并求和；任务卡若列出
   “用户已确认不涉及本次修改的告警数”，raw 合计减该数才是本单 `REMAINING_COUNT`。
   该数字来自主流程真实用户裁决，Agent 不得自行增减；无法区分时 raw 全量计入。
   不许只看最后一批，也不许只用首次数字减自认为修掉的数
5. 修复时同时避免制造新的常见问题：正式入参≤5（Python self/cls 不计）、有效代码行≤50
   （空行/纯注释/仅括号分隔行不计）、McCabe 圈复杂度≤5、本次修改代码行≤120字符。
   长行按项目 formatter 或附近同类代码换行，禁止机械拆字符串。提交前轻量检查由 Hook 静默触发；
   只处理任务卡精确范围内的高置信建议，最多复查两轮，异常/旧债直接放行。
6. commit 策略:完整流程有任何修复成功 → `git commit -m "[单号][类型]修复代码规范告警"`。
   独立模式任务卡写明“提交策略:禁止提交”时，**无论是否修复都不 commit**，保留工作区给用户检查。
7. 无法修复的整理成 REMAINING 清单后结束，不要按同一告警反复重启；**不要用抑制注释
