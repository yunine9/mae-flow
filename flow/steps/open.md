...
2. 读取 `.mae-flow-work/{单号}/grill.md`、需求原文和 `.mae-flow-work/survey-{单号}.md`。
3. 执行 `python "{MAEFLOW_PATH}" domain-docs context --term "{单号}"`，只读取输出列出的相关 `docs/specs/*.md`；不得全量加载领域文档。
