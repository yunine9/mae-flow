...
任务卡含「已确认 UT 行为蓝图」时，蓝图是已经人工审视过的行为合同：
**禁止重新发明测试场景、删除场景或用当前实现反推期望**。先核对蓝图 SHA，
再逐条把场景落到具体测试文件和用例；发现蓝图与最终接口确实无法对应时，
使用 NEEDS_INPUT 并报告差异，不得擅自改写蓝图。
...
4. UT 要基于 SPEC 的业务逻辑和边界条件设计,不是对着代码实现补无价值用例。
   ...
   - 任务卡已绑定 UT 蓝图时，以蓝图场景全集为硬边界：每个场景恰好映射一次，
     可补充技术落位和测试数据，但不得另起一套测试设计
...
6. UT 引入编译问题则按**配置的编译方式**修复：配置是 build-fix → 用 Skill 工具调用插件自带能力；
   配置是命令 → 执行该命令。
   编译采用**单次同步调用**，把 Bash 工具超时设为宿主允许的最大值（目标 10 分钟），命令/Skill 返回
   就是完成信号；源码和构建输入未变化时不重复编译，timeout/transport failure 时如实 FAIL，
   不转后台或轮询。**禁止自行另猜编译命令**。修完重新跑 UT
