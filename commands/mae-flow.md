...
grill / open / story 步骤给出确切指令,它们各自带着必需的输入与产出要求。
自行开局的调研既不进任何产物,也不构成任何步骤的证据,纯属重复劳动。
若宿主当前处于只读/计划模式(写不了文件,`init` 无法落盘),先告知用户切出该模式再开始;
不要用"先给个方案"代替流程。

1. Hook 会在项目根安装稳定入口 `.mae-flow-work/bin/mae-flow.py`；所有命令统一使用
   `python ".mae-flow-work/bin/mae-flow.py" ...`。若该文件客观缺失，只使用 Hook 注入的绝对脚本路径；
   禁止读取空环境变量、全盘搜索插件缓存或临时猜版本目录。通读 skills/mae-flow/SKILL.md 并全程遵守其铁律。
2. 按参数分流:
   - **moonlight / 月光宝盒** — 无人值守交付。若后续参数是 `report|repair|finalize|off`，直接执行对应
     `python ".mae-flow-work/bin/mae-flow.py" moonlight <动作>`；其中 report/repair 不询问用户，finalize 发现仍有遗留时才展示报告并
     等用户明确接受后携带原话重试。其他参数视为本次需求描述：
     - 从用户本条消息取真实出现的 `月光宝盒` 或 `moonlight` 作为短 `--ack`，执行
       `python ".mae-flow-work/bin/mae-flow.py" moonlight on --ack "<原词>"`；没有状态会自动初始化，有在途状态会从当前步骤切换；
     ...
     - 质量步骤真实尝试后仍失败，使用 current 输出的 `moonlight defer` 留痕继续，最终必须尝试 push；
       build 只有完整实现已形成且仅剩外部编译问题时才能 defer；push 后停在晨间检查，不自动归档领域文档；
   ...
   - **exit** — 退出当前在途流程、保留现有代码并改为普通开发。用户输入 `/mae-flow:mae-flow exit` 本身就是
     ...
     `python ".mae-flow-work/bin/mae-flow.py" exit --reason "用户明确执行 /mae-flow:mae-flow exit" --ack "/mae-flow:mae-flow exit"`；
   ...
   - **无参数** — 完整交付流程:项目根已有 `.mae-flow.json` 且 `current=end` → 直接 `init` 自动归档上一单
     并开启新轮；非终态才执行 `current` 续跑，禁止把已完成状态当成仍在途；
   ...
   - **ut** — 只补并执行单元测试，**不 init 完整流程**。如果项目还有 `.mae-flow.json` 在途状态，
     先说明不能叠加两套控制状态，让用户发送 `/mae-flow:mae-flow exit` 后重试；禁止自行退出。若状态已经
     `current=end`，它不算在途，`action start` 会先自动归档上一单再启动独立任务，不得要求用户 exit。
     从命令剩余文字提取目标文件/功能和验收说明；若用户只说了功能，先定向查找并定位至少一个被测
     业务文件。用重复的 `--files` 明确传入业务文件（可同时带相关测试文件），完整用户描述用
     `--request` 原样传入。执行：
     `python ".mae-flow-work/bin/mae-flow.py" action start ut --request "<用户描述>" --files "<被测业务文件>"`
     脚本会从项目预设或上次现场继承 UT 生成/运行/编译方式；缺项时只问缺失配置，不启动环境流程、不猜命令。
     `start` 只会冻结并展示文件清单，不会立即派 Agent。必须用 AskUserQuestion 让用户选择
     「确认以上范围 / 需要调整范围」；确认后执行
     `python ".mae-flow-work/bin/mae-flow.py" action confirm-scope`；命令会读取绑定当前范围指纹的新回答。调整则
   ...
   - **codecheck** — 只做代码规范检查和安全修复，**不 init 完整流程**。在途完整流程的处理同 ut。
     ...
     `python ".mae-flow-work/bin/mae-flow.py" action start codecheck --request "<用户描述>" [--files "<路径>"]`
   ...
   - **grill** — 只把需求问清楚并生成澄清结果，**不 init、不进入设计和编码**。在途完整流程的处理同 ut。
     执行 `python ".mae-flow-work/bin/mae-flow.py" action start grill --request "<用户原话>"`；已有文本材料用
   ...
   - **cancel / task cancel** — 仅取消当前独立 UT/CodeCheck/Grill 任务：
     `python ".mae-flow-work/bin/mae-flow.py" action cancel`。保留已经产生的代码和报告，不回滚，也不影响普通开发。
   - **chain** — 跨仓需求的链路分解,**不 init 流程**,先于任何仓的交付执行。由你(主模型)亲自做,
     ...
     ⑤ **逐仓反向核查**(落盘前,防"糊"):对每个仓自问——只看它的职责描述+契约,能独立开发吗?
       ...
       **结构按 `python ".mae-flow-work/bin/mae-flow.py" template chain` 输出的模板**(七章齐全;结构由 hook 硬校验,缺章节会被打回)。
   ...
   - **story** — 仅补生成 STORY,**不 init 流程**。单号按此顺序确定,拿不到就问,禁止瞎猜:
     ...
     ③ 都没有 → 问用户要单号。
     确定后先执行 `python ".mae-flow-work/bin/mae-flow.py" template` 拿模板绝对路径,再启动 story-generator-agent
     (模式=补生成,传入单号、该单的 change/archive 产物路径、模板绝对路径)。
     agent 的输出路径必须精确为 `.mae-flow-work/story/STORY-<单号>.md`；该目录是 Git 本地排除的过程区，
     STORY 永不进入交付提交；若写进 openspec、docs 或其他目录，视为生成失败并先移回过程区。
     agent 返回后照流程内 story 步的同一套确认纪律执行:
     待确认项用 AskUserQuestion 逐项拿用户拍板 → 你亲自把"(待确认)"改写为"(已确认)"(agent 无权)
     → 文档零"待确认"残留。最后展示本地路径与章节概览收尾，不再询问是否入库；
     交付中唯一允许进入仓库的文档是最终协调后的 `docs/specs/` 领域文档。
   - **review-fix** — 处理评审意见:本单已交付(MR 已建),处理评审/走读/流水线门禁意见。
     ...
     “是否重新启用”。先按状态唯一分流，禁止混用：
     - `.mae-flow.json` 的 `current=end` → **直接执行 `init`**，上一单会自动归档并开启新轮；不得先
       exit、goto、skip，也不得把终态误报成死锁。误用了 `init --new` 时脚本会自动归一化，不再卡住；
     - 只有项目存在 `.mae-flow.json.exited` 时，才执行 `messages` 找到本条命令的 ID，再执行
...
3. 此后所有流程动作只来自 `python ".mae-flow-work/bin/mae-flow.py" current` 的输出,禁止预判、禁止跳步。
