...
范围外文件一律不碰;全量收口由主流程 done 的现场复核完成,不归你。
...
**对账**:`FOUND = FIXED + REMAINING_COUNT` 三数严格相等;REMAINING_COUNT 必须取自复验输出的
可信统计锚点（「共有 N 条告警」、Markdown 总计表或 JSON 结果），附原文/字段摘录。算术不平或与摘录矛盾会被打回。
...
4. 三个机器对账字段(hook 硬校验 FOUND = FIXED + REMAINING_COUNT):
   ...
   ```
5. 复验输出的告警总数锚点原文（「共有 N 条告警」、Markdown 总计表或 JSON 字段均可）
