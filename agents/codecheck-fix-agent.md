...
1. 完整流程按基线拼文件清单；独立模式直接使用任务卡冻结的首检文件与告警，禁止扩大范围。
   执行 fullcheck → 读落盘报告拿告警清单
2. 逐条修复可自动修复的告警
3. 修复后**按配置的编译方式重新编译验证**(配置是 build-fix → 用 Skill 工具调用插件自带能力；
   是命令 → 执行该命令；
   长编译后台执行+**长间隔轮询**(单次调用内 sleep 后看日志,别把轮次烧在等待)——**禁止自行另猜编译命令**
4. **重跑 fullcheck 复验**。分批时逐批保留「共有 N 条告警」并求和；任务卡若列出
...
**对账**:`FOUND = FIXED + REMAINING_COUNT` 三数严格相等；`REMAINING_COUNT` 必须由复验输出的
可信统计锚点（「共有 N 条告警」、Markdown 总计表或 JSON 结果）按上述 raw/存量口径计算，
附每批原文/字段摘录及合计。算术不平或与摘录矛盾会被打回。
