...
_AGENT_EVIDENCE = AgentEvidenceRules(AgentEvidencePorts(
    moonlight=lambda state: _moonlight(state),
    step_entered=lambda state: _step_entered_at(state),
    risk_acceptance=lambda kind, state: _risk_acceptance(kind, state),
    script_path=lambda: sys.argv[0],
    risk_labels=RISK_AGENT_LABELS,
    tokens=lambda: _agent_token_data(),
    rejections=lambda: _agent_rejection_data(),
    source_snapshot_since=lambda head, state: _source_snapshot_since(
        head, state),
    source_changed_since=lambda head, state: _source_changed_since(
        head, state),
    changed_source_files=lambda state: _changed_source_files(state),
    shell_output=lambda command: sh(command),
    argv_output=lambda arguments: argv_out(arguments),
    blocking_dirty_source_paths=lambda state: _blocking_dirty_source_paths(
        state),
))
...
_DELIVERY_EVIDENCE = DeliveryEvidenceRules(DeliveryEvidencePorts(
    moonlight=lambda state: _moonlight(state),
    development_review=lambda state: _development_review(state),
    source_changed_since=lambda head, state: _source_changed_since(
        head, state),
    review_before_commit=lambda data: _review_before_commit(data),
    final_review_delta=lambda state: _final_review_delta(state),
    archive_delivery_paths=lambda state: _archive_delivery_paths(state),
    shell_output=lambda command: sh(command),
    argv_output=lambda arguments: argv_out(arguments),
    committed_initial_carryover=lambda state: _committed_initial_carryover(
        state),
    committed_delivery_paths=lambda state: _committed_delivery_paths(state),
    trusted_harness_commit_path=lambda path, state:
        _trusted_harness_commit_path(path, state),
    dirty_paths=lambda: _dirty_paths(),
    path_fingerprint=lambda path: _path_fingerprint(path),
    repo_path_identity=lambda path: _repo_path_identity(path),
    agent_written_paths=lambda: _agent_written_paths(),
...
_QUALITY_EVIDENCE = QualityEvidenceRules(QualityEvidencePorts(
    business_changed_files=lambda state: _biz_changed_files(state),
    risk_acceptance=lambda kind, state: _risk_acceptance(kind, state),
    source_changed_since=lambda head, state: _source_changed_since(
        head, state),
    agent_ran=lambda spec, state: _AGENT_EVIDENCE.agent_ran(spec, state),
    ...
    git_head=lambda: sh("git rev-parse --verify HEAD"),
    ...
    exemption_text_has_pair=lambda text, rule, path:
        _exemption_text_has_pair(text, rule, path),
    approved_exemptions=lambda state: _approved_exemptions(state),
    was_exempt_before_review=lambda state, exemption, rule, path:
        _was_exempt_before_review(state, exemption, rule, path),
    approval_key=lambda rule, path: _approval_key(rule, path),
))
...
def _checkpoint_recovery_ports(st):
    return CheckpointRecoveryPorts(
        ...
        merge_base=lambda base, head: argv_out([
            "git", "merge-base", base, head]),
        commit_paths=lambda base: tuple(argv_out([
            "git", "-c", "core.quotepath=false", "diff",
            "--name-only", "--no-renames", base, "HEAD",
        ]).splitlines()),
        commit_count=lambda base: argv_out([
            "git", "rev-list", "--count", base + "..HEAD"]),
        dirty_paths=lambda: tuple(_dirty_paths()),
        commit_tagged=lambda: ev_commit_tagged({}, st),
        ...
        reopen_spec_archive=lambda state: _reopen_spec_archive(
            state),
    )
...
def _apply_checkpoint_refresh(st, result):
    ...
    if show_checkpoint:
        data = _development_review(st)
        _show_checkpoint_review(st, data, _checkpoint_current(st))
    if show_final:
        data = _development_review(st)
    ...
    for effect in result.effects:
        if effect.kind == "refresh_final_review":
            refreshed = refresh_final_review(
                st, _checkpoint_recovery_ports(st))
            _apply_checkpoint_refresh(st, refreshed)
            return
        if effect.kind == "refresh_checkpoint":
            refreshed = refresh_checkpoint(
                data, st.get("current"),
                _checkpoint_recovery_ports(st))
            _apply_checkpoint_refresh(st, refreshed)
            return
        raise RuntimeError(
    ...
    for effect in result.effects:
        if effect.kind == "set_development_review":
            ...
            migration = refresh_final_review(
                st, _checkpoint_recovery_ports(st))
            _apply_checkpoint_refresh(st, migration)
            migrated = True
        elif effect.kind == "show_final_review":
    ...
    for effect in result.effects:
        if effect.kind == "set_development_review":
            ...
            rework = activate_final_rework(
                st, payload, _checkpoint_recovery_ports(st))
            _apply_checkpoint_refresh(st, rework)
            return
        else:
            raise RuntimeError(
...
def _moonlight_build_defer_boundary(st):
    for evaluator in (
            ev_tasks_checked, ev_commit_tagged_after_entry):
        ok, why = evaluator({}, st)
        if not ok:
            return False, why
    return True, ""
...
def _moonlight_blocked(flow, st, args):
    can_block = _moonlight_can_block(st["current"])
    step_kind = _moonlight_step_kind(st["current"])
    ...
    result = record_blocker(
        ...
        head=sh("git rev-parse --verify HEAD"),
        dirty_paths=tuple(_dirty_paths()[:100]),
        now=time.strftime("%Y-%m-%d %H:%M:%S"),
    )
...
def _moonlight_finalize(flow, st, args):
    ...
    if step_validation.exit_code:
        die(step_validation.stderr[0], step_validation.exit_code)
    issues = _moonlight_unresolved(st)
    ack_verified = (
        _ack_verified(st, args.ack, exact=True)
...
def cmd_moonlight(flow, st, args):
    ...
    if action == "report":
        ...
        return
    if action == "off":
        enabled = _moonlight(st)
        ack_verified = (
            _ack_verified(st, args.ack, exact=False)
        ...
        result = defer_moonlight_quality(
            ...
            ports=MoonlightDeferPorts(
                ...
                dirty_paths=lambda: tuple(
                    _blocking_dirty_source_paths(st, flow)),
                head=lambda: sh(
                    "git rev-parse --verify HEAD"),
                ...
                source_changes=lambda: (
                    _business_source_changed_since_step(
                        st, sid)),
    ...
    if action == "repair":
        ...
        needs_head = bool(
            not _moonlight_data(st).get("hard_blocked")
