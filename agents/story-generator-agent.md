...
4. 严格按实施附录模板生成 `.mae-flow-work/<单号>/implementation.md`；逐条把 Grill 决策追踪到实现影响，并在此记录关键函数修改、轻量 CP、风险回滚和领域归档影响。
5. 实施附录中的 CP 只保留轻量业务边界和完成合同，不生成 Roadmap、Test Blueprint 或逐行 Build Plan。
6. 两个文件均为本地过程件，不得写入 `docs/story/`、`openspec/` 或其他目录。
