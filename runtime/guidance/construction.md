...
- After review fixes, invoke `compile-agent` synchronously once for this CP and pass the exact CP, Intake-confirmed Build route, Build directory, and changed production files. The subagent invokes `build-fix` for configured C++ projects or the confirmed Maven/repository command for other projects. A direct main-agent Skill/Bash call is not a CP Build receipt. Never use delay loops, repeated status probes, detached execution, or automatic Build retry.
