...
暂停或普通修改指令**，也不要继续执行 current。先运行 `mae-flow.py exit` 展示当前步骤、分支、HEAD、
未提交文件和退出影响，再用 AskUserQuestion 确认；不可用时结束回复等用户用普通消息确认。
拿到确认后原文复制执行：

```
python "<插件目录>/scripts/mae-flow.py" exit --reason "<具体原因>" --ack "<用户确认原话>"
```
