...
- [ ] Design 的 Story 按模板同时承载客户场景、性能规格、功能验收、软件详细设计和测试交接；性能规格不重复业务行为 Spec，接口设计不收纳内部函数/方法；它不是逐行编码计划。Story/Reviewer fake 各只调用一次，Reviewer 普通意见修正后由用户确认最终 Story，不透明返回后停在 Design 用户卡。
