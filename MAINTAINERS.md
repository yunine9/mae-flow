...
**CodeCheck/UT 覆盖口径（2026-07-26 用户拍板）**：检查/测试对象=**本次修改的函数**，不是整个变更文件——一单不背存量债。实现：`_changed_lines`（git diff -U0 的 +侧行集合）是范围数据源；CodeCheck 告警按变更行 ±`CODECHECK_LINE_SLACK`(3) 窗口过滤（scan 与 done 复核同源同口径，存量数如实展示；明细缺行号时保守全算并明示）；UT 任务卡携带"本次修改行范围"，agent 契约禁止为未修改的存量函数补测。窗口外扩是"改动所在函数"的近似——函数级规则（超长/圈复杂度）告警常报在签名行。
