...
确实没有可复用产出时再把重跑耗时与不重跑风险一起交给用户选择。用户选重跑才重启 agent（附上次异常信息）;
选承担风险才按报错执行 `accept-risk story`。**禁止你亲自代写 STORY**;文档残留待确认、未提交等其他检查仍会拦截。
...
STORY 完成后进入编码计划检视；不要直接生成检查点或修改业务源码。
