...
「# 实现清单」只列生产代码/配置实现，不列 UT、测试文件或测试用例；UT 由 verify_ut
按已确认蓝图统一生成和运行。
...
若②③类恰好为 0,不要临场发明风险项或用 accept-risk 制造假放行记录；改用一次
AskUserQuestion 展示①类决策摘要卡供抽查,选项固定为「通过并进入设计 / 抽查第 N 条边界值 /
需要调整」。这仍是对规格合同的有价值审阅,同时满足本步 ASKUSER 硬证据。
