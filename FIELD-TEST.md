...
- [ ] **1.1 环境初始化链**:自动安装(内网镜像/代理)→ comet init BLOCKED 时给全三要素(目录/命令/平台)→
  手动跑完只回"好了"是否**自动重派** env-setup-agent → .claude→.cac 迁移 → /reload-skills 提示 → 重启提示。
  **顺带验证 gate**:让 AI 试跑 `comet init` 应被硬拦(2026-07-20 实战修复:自动化执行会初始化全部平台)。
  若此前误自动化留下了多平台目录(.cursor/.windsurf 等):按 agent 的污染检测提示清单,由人删除。
...
- [ ] **2.4 codecheck 独立 CLI 探查**:公司环境有无不依赖会话的 codecheck 命令行。
  有 → 记录命令与输出格式,回头加 `codecheck_clean` 证据类型(done 现场重跑,最硬约束)。
