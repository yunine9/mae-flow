...
如果 CP 内 compile-agent 已长时间执行但令牌因宿主/收尾兼容问题始终签不出,不要自动无限重跑；
真实 build-fix/编译命令已经完成、只是最终报告格式被打回时，只修报告，Hook 会复用绑定当前任务卡、
编译路由和源码快照的 COMPILE 凭证。确实没有可验证执行凭证时，把「无可验证编译结果,代码可能无法构建」
的风险告知用户，由用户选择重跑或按门禁提示执行 `accept-risk compile`。分阶段 CP 的风险确认绑定当前
未提交源码快照，成功后执行 `checkpoint ready CPn`；其他步骤仍重新执行 `done`。后续代码变化自动失效。
