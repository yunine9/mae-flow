...
def _emit_items(items, index, indent, out):
    ...
    while index < len(items) and items[index][0] >= indent:
        if items[index][0] > indent or _kind(items[index][1]) != kind:
            index = _emit_items(items, index, items[index][0], out)
            continue
        out.append("<li>%s" % inline(items[index][2]))
        index += 1
...
def _list(items):
    out = []
    _emit_items(items, 0, items[0][0], out)
    return "".join(out)
...
def _collect_list(lines, index):
    ...
    while index < len(lines):
        ...
        if match:
            ...
            last = items[-1]
            items[-1] = (last[0], last[1], last[2] + " " + lines[index].strip())
            index += 1
        else:
            break
    return items, index
...
def render(text, fence_hook=None):
    """markdown → HTML。
    ...
    while index < len(lines):
        ...
        if line.startswith("```"):
            ...
            out.append(taken if taken is not None
        ...
        if head:
            ...
            out.append("<h%d>%s</h%d>"
                       % (level, inline(head.group(2).strip()), level))
            index += 1
            continue
