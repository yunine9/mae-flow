...
备课前从需求中提取领域关键词，执行 `python "{MAEFLOW_PATH}" domain-docs context --term "<需求关键词>"`，
只读取输出列出的相关领域文档；禁止全量扫描 `docs/specs/`。索引报错时先修复并执行
`python "{MAEFLOW_PATH}" domain-docs validate`，通过后继续，不得循环猜测。
