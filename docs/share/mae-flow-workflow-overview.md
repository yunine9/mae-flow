...
全部代码编译通过以后，普通模式会先停下来，把本轮提交、改动文件和完整差异范围展示给用户。用户可以认真读代码，也可以按给出的步骤亲自验证；只有明确确认继续，才进入后面的检查和测试。发现问题则回到编码环节，修改后重新编译和检视。月光宝盒不进入这个人工停靠点。
...
准备项目 → 问清需求 → 确认需求说明 → 设计方案 → 编码编译 → 用户检视代码 → 检查和测试 → 确认并推送
