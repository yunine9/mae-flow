...
class StableRecoveryResult:
    """Conservative semantic projection from Lean v3 to stable v2."""
...
def recover_lean_flow(raw):
    """Project only durable semantics from strict Lean v3 into stable v2.

    Agent tokens, task cards, hashes, reviewer digests, fingerprints and Git
    receipts have no destination by construction.
    """
    ...
    boundary = _SAFE_BOUNDARY_BY_PHASE.get(phase)
    if not boundary:
